// This file is how TestRunner.html tests your neanderthal code.
// You shouldn't need to modify anything here, but you could check
// out failing tests to see what exactly is expected of your code.
...
describe('01 The Math Object', function() {
  ...
  it('floor should round numbers down', function() {
    ...
    expect(floor(10.75)).to.equal(10);
    expect(floor(-3.33)).to.equal(-4);
...
describe('02 Array Methods', function() {
  var array;
  ...
  it('shift should remove and return the first item in an array', function() {
    ...
    expect(array.length).to.equal(0);
    expect(shift([])).to.equal(undefined);

  });

  it('unshift should add any number of items to the beginning of an array', function() {

    dontCheatOn(unshift);
    expect(unshift(array, 'kumqwat')).to.equal(5);
    expect(array[0]).to.equal('kumqwat');
    expect(array.length).to.equal(5);
    expect(unshift(array, 42, false, ['c'])).to.equal(8);
    expect(array).to.deep.equal([42, false, ['c'], 'kumqwat', -1, true, 'abc', 0.5]);
    expect(array.length).to.equal(8);
    expect(unshift([], 1, 2, 3)).to.equal(3);
...
describe('03 String methods', function() {
  ...
  it('trim should remove white space to either side of a string', function() {
    ...
    expect(trim(' \t\tDear Esther, \n\tYou\'re alright.')).to.equal('Dear Esther, \n\tYou\'re alright.');
...
describe('04 Extra Credit', function() {
  ...
  it('split should turn a string into an array based on a seperator', function() {
  ...
  it('parse should take a date string and return the milliseconds since January 1, 1970', function() {
    ...
    expect(parse('2016-03-31T01:27-06:00')).to.equal(1459409220000);
    expect(parse('23 Jun 1912')).to.equal(-1815350400000);
    expect(parse('14 Jul 2015 11:49 +00:00')).to.equal(1436874540000);
    expect(parse('December 12, 1990')).to.equal(660960000000);
    expect(parse('1902-01-00')).to.be.NaN;
